      ...
      (async () => {
        ...
        for (const command of listCommand) {
          try {
            ...
            if (!config?.name) {
              try {
                throw new Error(`[ EDUC V2 ] ${command} command has no name property or empty!`);
              } catch (error) {
            ...
            if (!config?.commandCategory) {
              try {
                throw new Error(`[ EDUC V2 ] ${command} commandCategory is empty!`);
              } catch (error) {
                console.log(chalk.red(error.message));
                continue;
              }
            ...
            if (!config?.hasOwnProperty('usePrefix')) {
              console.log(`EDUC V2`, chalk.hex("#ff0000")(command) + ` does not have the "usePrefix" property.`);
              continue;
            }

            if (global.client.commands.has(config.name || '')) {
              console.log(chalk.red(`[ EDUC V2 ] ${chalk.hex("#FFFF00")(command)} Module is already loaded!`));
              continue;
            }
            ...
            if (dependencies) {
              Object.entries(dependencies).forEach(([reqDependency, dependencyVersion]) => {
                ...
                try {
                  ...
                  const errorMessage = `[EDUC V2] Failed to install package ${reqDependency} for module`;
            ...
            try {
              global.loading(`${cra(`EDUC V2`)} ${cb(config.name)} success`, "EDUC V2");
            } catch (err) {
            ...
            global.loading.err(`${chalk.hex('#ff7100')(`EDUC V2`)} ${chalk.hex("#FFFF00")(command)} fail ` + error, "EDUC V2");
          }
        }
      })(),
        ...
        for (const ev of events) {
          try {
            ...
            if (!config || !config.name || !run) {
              global.loading.err(`${chalk.hex('#ff7100')(`LOADED`)} ${chalk.hex("#FFFF00")(ev)} Module is not in the correct format. `, "EDUC V2");
              continue;
            }
            ...
            if (global.client.events.has(config.name)) {
              global.loading.err(`${chalk.hex('#ff7100')(`EDUC V2`)} ${chalk.hex("#FFFF00")(ev)} Module is already loaded!`, "EDUC V2");
              continue;
            }
            ...
            global.loading(`${cra(`EDUC V2`)} ${cb(config.name)} success`, "EVENT");
          }
          catch (err) {
    ...
    global.handleListen = loginApiData.listenMqtt(async (error, message) => {
      if (error) {
        if (error.error === 'Not logged in.') {
          logger("Your bot account has been logged out!", 'EDUC V2');
          return process.exit(1);
        }
        if (error.error === 'Not logged in') {
          logger("Your account has been checkpointed, please confirm your account and log in again!", 'EDUC V2');
          return process.exit(0);
        }
        console.log(error);
        return process.exit(0);
...
(async () => {
  try {
    console.log(cv(`\n` + `──DATABASE─●`));
    global.loading(`${cra(`[ CONNECT ]`)} Connected to JSON database successfully!`, "EDUC V2");
    onBot();
    } catch (error) {
       global.loading.err(`${cra(`[ CONNECT ]`)} Failed to connect to the JSON database.`, "EDUC V2");
      }
})();
