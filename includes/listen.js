  ...
  const sky = gradient('#3446eb', '#3455eb', '#3474eb');
	...
	(async function () {
		try {
			...
			logger.loader(`Successfully loaded ${chalk.blueBright(`${global.data.allThreadID.length}`)} threads and ${chalk.blueBright(`${global.data.allUserID.length}`)} users`);
		} catch (error) {
			logger.loader(`Can't load environment variable, error: ${error}`, 'error');
		}
	})();	
	global.loading(`${crayon(`[ BOT_INFO ]`)} success!\n${sky(`[ NAME ]:`)} ${(!global.config.BOTNAME) ? "Bot Messenger" : global.config.BOTNAME} \n${sky(`[ FBID ]:`)} ${api.getCurrentUserID()} \n${sky(`[ PRFX ]:`)} ${global.config.PREFIX}`, "LOADED");
