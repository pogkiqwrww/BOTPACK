...
function startServer(port) {
  ...
  app.on('error', (error) => {
    logger(`An error occurred while starting the server: ${error}`, "EDUC V2");
  });
}
 
async function startBot(index) {
  logger(`Getting Started!`, "EDUC V2");
  try {
    ...
    child.on("error", (error) => {
      logger(`An error occurred while starting the child process: ${error}`, "EDUC V2");
    });
  } catch (err) {
    logger(`Error while starting the bot: ${err}`, "EDUC V2");
  }
}